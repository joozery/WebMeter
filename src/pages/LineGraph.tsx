...
import { Search, Printer, FileText, Image as ImageIcon } from 'lucide-react';
...
export default function LineGraph() {
  ...
  async function handleExport(type: 'pdf' | 'csv' | 'image') {
    ...
    const baseName = `LineGraph-${fileDate}`;
  ...
  // ฟังก์ชันสร้าง PDF จากกราฟ
  const generateGraphPDF = async () => {
    if (!isLoaded || chartData.length === 0) {
      alert('กรุณาโหลดข้อมูลก่อนทำการพิมพ์');
      return;
    }
    ...
    try {
      ...
      if (!chartElement) {
        alert('ไม่สามารถดึงข้อมูลกราฟได้');
        return;
      }
      ...
      doc.text('WebMeter - Line Graph Report', 14, 20);
      ...
      doc.text(`Date Range: ${fromDate} ${timeFrom} - ${toDate} ${timeTo}`, 14, 35);
      doc.text(`Type: ${typeFilter === '24h' ? 'Day' : typeFilter === '1month' ? 'Month' : 'Year'}`, 14, 40);
      doc.text(`Chart Type: ${graphType === 'line' ? 'Line Chart' : 'Bar Chart'}`, 14, 45);
      doc.text(`Generated: ${format(now, 'dd/MM/yyyy HH:mm:ss')}`, 200, 30);
      doc.text(`Data Points: ${chartData.length}`, 200, 35);
      doc.text(`Columns: ${displayColumns.length}`, 200, 40);
      
      // คำนวณขนาดรูปภาพที่เหมาะสม
      const pageWidth = doc.internal.pageSize.getWidth();
      const pageHeight = doc.internal.pageSize.getHeight();
      const imgWidth = pageWidth - 28; // margin 14 * 2
      const imgHeight = (canvas.height * imgWidth) / canvas.width;
      
      // วางรูปภาพลงใน PDF
      const yPosition = 55;
      let maxImageHeight = pageHeight - yPosition - 60; // เหลือพื้นที่สำหรับข้อมูลด้านล่าง
      
      if (imgHeight > maxImageHeight) {
        // ถ้ารูปใหญ่เกินไป ให้ปรับขนาด
      ...
      // เพิ่มข้อมูลสรุปการแสดงผล
      let summaryY = yPosition + maxImageHeight + 15;
      
      // เส้นแบ่ง
      doc.setLineWidth(0.5);
      doc.line(14, summaryY - 5, pageWidth - 14, summaryY - 5);
      
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text('Data Summary', 14, summaryY);
      
      // แสดงรายการคอลัมน์ที่แสดงผล
      summaryY += 10;
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.text(`Displayed Columns: ${displayColumns.join(', ')}`, 14, summaryY);
      
      // หาค่าสถิติของแต่ละคอลัมน์ที่แสดง
      if (chartData.length > 0) {
        summaryY += 10;
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text('Statistics:', 14, summaryY);
        
        // แสดงสถิติในรูปแบบตาราง
        const statsPerRow = 2; // แสดง 2 คอลัมน์ต่อแถว
        let currentRow = 0;
        let currentCol = 0;
        
        displayColumns.forEach((column, index) => {
          const values = chartData.map(row => row[column]).filter(val => typeof val === 'number' && !isNaN(val));
          if (values.length > 0) {
            const min = Math.min(...values);
            const max = Math.max(...values);
            const avg = values.reduce((sum, val) => sum + val, 0) / values.length;
            
            const xPos = 14 + (currentCol * 140);
            const yPos = summaryY + 8 + (currentRow * 8);
            ...
            doc.text(`Min: ${min.toFixed(2)}`, xPos, yPos + 3);
            doc.text(`Max: ${max.toFixed(2)}`, xPos, yPos + 6);
            doc.text(`Avg: ${avg.toFixed(2)}`, xPos, yPos + 9);
            
            currentCol++;
            if (currentCol >= statsPerRow) {
              currentCol = 0;
              currentRow += 3; // เว้นพื้นที่สำหรับ 3 บรรทัดข้อมูล + เว้นว่าง
            }
          }
        });
      }
      ...
      // บันทึกไฟล์ PDF
      const defaultFileName = `${now.getDate().toString().padStart(2, '0')}${(now.getMonth() + 1).toString().padStart(2, '0')}${thaiYear}`;
      const fileName = `LineGraph_${meterName.replace(/[^a-zA-Z0-9]/g, '_')}_${defaultFileName}_${now.getHours().toString().padStart(2, '0')}${now.getMinutes().toString().padStart(2, '0')}.pdf`;
      doc.save(fileName);
  ...
  const prepareChartData = () => {
    ...
    console.log('🏢 Selected Slave IDs:', selectedSlaveIds);
    console.log('📊 Selected Meter Names:', selectedMeterNames);
      ...
      sortedTable.forEach(row => {
        ...
        selectedSlaveIds.forEach((slaveId, index) => {
          ...
          displayColumns.forEach(column => {
            ...
            if (typeof value === 'string' && !isNaN(parseFloat(value))) {
              dailyData[day][dataKey] = (dailyData[day][dataKey] || 0) + parseFloat(value);
            } else if (typeof value === 'number' && !isNaN(value)) {
              dailyData[day][dataKey] = (dailyData[day][dataKey] || 0) + value;
            }
          });
        });
          ...
          if (!monthlyData[month]) {
           ...
           selectedSlaveIds.forEach((slaveId, index) => {
             ...
             displayColumns.forEach(column => {
               ...
               const value = row[column];
               if (typeof value === 'string' && !isNaN(parseFloat(value))) {
                 monthlyData[month][dataKey] = (monthlyData[month][dataKey] || 0) + parseFloat(value);
               } else if (typeof value === 'number' && !isNaN(value)) {
                 monthlyData[month][dataKey] = (monthlyData[month][dataKey] || 0) + value;
               }
             });
           });
    ...
    // สำหรับ day view ใช้ logic เดิม
      ...
      console.log('📊 Day view - ข้อมูลที่ส่งเข้า:', tableData);
      console.log('📊 Day view - จำนวนข้อมูล:', tableData.length);
        ...
        if (closestRow && minDiff <= 7) {
          ...
          const chartRow: any = {
           ...
           selectedSlaveIds.forEach((slaveId, index) => {
             ...
             displayColumns.forEach(column => {
               const dataKey = `${meterName} - ${column}`;
               const value = closestRow.row[column];
               if (typeof value === 'string' && !isNaN(parseFloat(value))) {
      ...
      console.log('📊 Day view - จำนวนข้อมูลที่ประมวลผล:', processedRows.length);
  ...
  // useEffect สำหรับโหลดข้อมูลเมื่อเลือกมิเตอร์
  useEffect(() => {
              ...
              {isLoading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground">{isLoading ? (language === 'TH' ? 'กำลังโหลด...' : 'Loading...') : (language === 'TH' ? 'โหลด' : 'Load')}</p>
                  </div>
                </div>
