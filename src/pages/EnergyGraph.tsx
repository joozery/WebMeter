...
export default function EnergyGraph() {
  ...
  async function handleExport(type: 'pdf' | 'csv' | 'image') {
    ...
    const baseName = `EnergyGraph-${fileDate}`;
  ...
  const loadChartData = async () => {
    ...
    try {
      ...
      console.log('📊 Selected Columns:', contextSelectedColumns);
        ...
        console.log('⏰ เปลี่ยนระยะห่างการแสดงข้อมูลเป็น 1 ชั่วโมง');
  ...
  const prepareChartData = () => {
    ...
    // สำหรับ day view ใช้ logic เดิม
      ...
      // สร้าง array ของ targetTimes โดยบวกทีละ 1 ชั่วโมง
      const startMinutes = parseTimeToMinutes(timeFrom);
      const endMinutes = parseTimeToMinutes(timeTo);
      ...
      while (t <= endMinutes) {
        targetTimes.push(t);
        t += 60; // เปลี่ยนจาก 15 นาที เป็น 60 นาที (1 ชั่วโมง)
      }
      ...
      for (const targetMinute of targetTimes) {
        ...
        if (closestRow && minDiff <= 30) { // เพิ่ม tolerance จาก 7 นาที เป็น 30 นาที เพื่อให้เหมาะสมกับช่วงเวลา 1 ชั่วโมง
  ...
  const getXAxisConfig = () => {
    if (typeFilter === '1year') {
      ...
      // สำหรับ Day view ให้แสดงแกน X เป็น 0-24 เสมอ (ทุก 1 ชั่วโมง)
      return {
        domain: [0, 24],
